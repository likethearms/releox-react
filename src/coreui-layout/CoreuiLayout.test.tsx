...
describe('CoreuiLayout', () => {
  describe('CoreuiHeader', () => {
    it('should set logo redirect link to / by default', () => {
      wrapper = mount(routerWrapper(<CoreuiLayout brand="Foo Bar" sidebarMenu={[]}>MainContent</CoreuiLayout>));
      expect(wrapper.find('[to="/"]')).toHaveLength(1);
    });

    it('should set logo redirect link to /foo', () => {
      wrapper = mount(routerWrapper(<CoreuiLayout brand="Foo Bar" sidebarMenu={[]} brandUrl="/foo">MainContent</CoreuiLayout>));
      expect(wrapper.find('[to="/foo"]')).toHaveLength(1);
    });

    it('should show title', () => {
      wrapper = mount(routerWrapper(<CoreuiLayout brand="Foo Baz" sidebarMenu={[]}>MainContent</CoreuiLayout>));
    ...
    it('should add class to app class when toggle clicked', () => {
      ...
      const onClick = wrapper.find('.CoreuiHeaderSideBarToggleComponent').prop('onClick') as Function;
      onClick();
